// --- Botón de modo oscuro + Bloque de búsqueda ---
document.addEventListener('DOMContentLoaded', () => {
    const body = document.body;
    const themeToggleCheckbox = document.getElementById('theme-toggle-checkbox');
    const searchInput = document.getElementById('search-input'); // input de búsqueda
    const items = document.querySelectorAll('.config-section');  // elementos a filtrar
...
function setupEvents() {
    ...
    tabsContainer.addEventListener('click', (e) => {
        ...
        // Quitar la clase 'active' de todas las pestañas y su contenido
        tabs.forEach(tab => tab.classList.remove('active'));
        tabContents.forEach(content => content.classList.remove('active'));

        // Agregar la clase 'active' a la pestaña y contenido seleccionados
        clickedTab.classList.add('active');
        const activeTabContentId = clickedTab.dataset.tab + "-tab";
    ...
    categoriesList.addEventListener('click', function(e) {
        const targetBtn = e.target.closest('.edit-category, .delete-category');
        if (!targetBtn) return;
        
        const index = parseInt(targetBtn.getAttribute("data-index"));
        
        if (targetBtn.classList.contains('edit-category')) {
            editCategory(index);
        } else if (targetBtn.classList.contains('delete-category')) {
            showConfirmModal(
                "Eliminar Categoría",
                `¿Está seguro que desea eliminar la categoría "${systemParams.categories[index]}"?`,
                "deleteCategory",
                index
            );
        }
    ...
    statusesList.addEventListener('click', function(e) {
        const targetBtn = e.target.closest('.edit-status, .delete-status');
        if (!targetBtn) return;
        
        const index = parseInt(targetBtn.getAttribute("data-index"));
        
        if (targetBtn.classList.contains('edit-status')) {
            editStatus(index);
        } else if (targetBtn.classList.contains('delete-status')) {
            showConfirmModal(
                "Eliminar Estado",
                `¿Está seguro que desea eliminar el estado "${systemParams.statuses[index].name}"?`,
                "deleteStatus",
                index
            );
        }
